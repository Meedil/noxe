...
export default function SearchResults() {
  ...
  const content = contentData.results
    .filter((item) => item.gender === undefined)
    .map((movieDetails) => {
        ...
        <span className="float-end">
          <div className={"d-inline-block " + (disablePrev ? "disabled" : "")}>
            <Link
              to={
                !disablePrev
                  ? `/searchResults/${searchText}/${parseInt(pageNum) - 1}`
                  : ""
              }
              className={"prev-pageNum d-inline-block"}
            >
              <CaretLeftFill color={disablePrev ? "#6c757d" : "white"} />
            </Link>
          </div>
          Page <span className="subtext">{pageNum}</span> /{" "}
          <Link to={`/searchResults/${searchText}/${contentData.total_pages}`}>
            {" "}
          ...
          <Link
            ...
            className="prev-pageNum d-inline-block"
          >
            <CaretRightFill color={disableNext ? "#6c757d" : "white"} />
          </Link>
        </span>
      </div>
