      ...
      <div className="fs-4 mb-3">
        ...
        <span className="float-end">
          <div className={"d-inline-block " + (disablePrev ? "disabled" : "")}>
            <Link
              to={!disablePrev ? `/discover/${type}/${parseInt(page) - 1}` : ""}
              className={"prev-page d-inline-block"}
            >
              <CaretLeftFill color={disablePrev ? "#6c757d" : "white"} />
            </Link>
          </div>
          Page <span className="subtext">{page}</span> /{" "}
          <Link to={`/discover/${type}/${contentData.total_pages}`}>
            {" "}
            {contentData.total_pages}
          </Link>
          <Link
            to={!disableNext ? `/discover/${type}/${parseInt(page) + 1}` : ""}
            className="prev-page d-inline-block"
          >
