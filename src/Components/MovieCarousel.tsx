...
export default function MovieCarousel({ moviesTrendingTodayResponse }) {
  ...
  useEffect(() => {
    ...
    moviesTrendingTodayResponse
      .then((response) => response.json())
      .then((movies) => {
        console.log("response: ", movies);

        setMovieSlides(
          movies.results.map((movieDetails, index) => {
            return (
              <Link
                key={movieDetails.id}
                to={`/movieDetails/movie/${movieDetails.id}`}
              >
                <div
  ...
  return (
    <div
      ...
      className="carousel slide mb-md-5"
      data-bs-ride="false"
    >
      <div className="carousel-inner w-sm-75 mx-auto">{movieSlides}</div>
      <button
